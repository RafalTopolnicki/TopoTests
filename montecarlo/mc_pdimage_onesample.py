...
from pdimage import PDImageTest_onesample
import pandas as pd
import logging
...
def run_mc(rvs, args):
    ...
    pdimage_test = PDImageTest_onesample(
        n=args.n,
        dim=args.dim,
        significance_level=args.alpha,
    ...
    for rv_alter in rvs:
        ...
        tt_out = pdimage_test.predict(samples)
        results.append(
            data_row(
                args=args,
                ...
                accpecth0=tt_out[0],
                pvals=tt_out[1],
                threshold=pdimage_test.representation_threshold,
            )
        )
