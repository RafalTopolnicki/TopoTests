...
from topotests.distancetest import DistanceTest_onesample
import pandas as pd
import logging
...
def run_mc(rvs, args):
    outputfilename = f"distancetest_dim={args.dim}_n={args.n}_norm={args.norm}_method={args.method}.csv"
    ...
    for rv_true in rvs:
        ...
        distance_test = DistanceTest_onesample(
            n=args.n,
            dim=args.dim,
            method=args.method,
            norm=args.norm,
        ...
        # train TopoTest
        distance_test.fit(rv=rv_true, n_signature=args.n_signature, n_test=args.n_test)
        for rv_alter in rvs:
            if df_done is not None and np.any(np.logical_and(df_done.true_dist == rv_true.label, df_done.alter_dist == rv_alter.label)):
                logging.info(f"DT-1s: Skip distribution true: {rv_true.label} alter: {rv_alter.label}")
                continue
            else:
                logging.info(f"DT-1s: Start distribution true: {rv_true.label} alter: {rv_alter.label}")
                ...
                tt_out = distance_test.predict(samples)
                results.append(
                    data_row(
                        args=args,
                        ...
                        accpecth0=tt_out[0],
                        pvals=tt_out[1],
                        threshold=distance_test.representation_threshold,
                    )
                )
