...
from pdllmeanpdftest import PDLLMeanPDFTest_onesample
import pandas as pd
import logging
...
def run_mc(rvs, args):
    ...
    outputfilename = f"dpllmeanpdftest_dim={args.dim}_n={args.n}_persistence_dim={args.persistence_dim}_null={null_distr_label}.csv"
    outputfilepath = os.path.join(args.output_dp, outputfilename)
    results = []

    logging.info(f"LLMeanPDF-1s: Start true distribution: {rv_true.label} n={args.n} dim={args.dim} persistence_dim={args.persistence_dim}")

    pdlll_test = PDLLMeanPDFTest_onesample(
        n=args.n,
        dim=args.dim,
        significance_level=args.alpha,
    ...
    for rv_alter in rvs:
        logging.info(f"LLMeanPDF-1s: Start distribution true: {rv_true.label} alter: {rv_alter.label}")
