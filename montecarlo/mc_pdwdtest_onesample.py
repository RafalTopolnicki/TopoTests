...
def run_mc(rvs, args):
    ...
    outputfilename = f"pdwdtest_dim={args.dim}_n={args.n}_persistence_dim={args.persistence_dim}_null={null_distr_label}.csv"
    outputfilepath = os.path.join(args.output_dp, outputfilename)
    results = []

    logging.info(f"PDWD-1s: Start true distribution: {rv_true.label} n={args.n} dim={args.dim} persistence_dim={args.persistence_dim}")
    ...
    for rv_alter in rvs:
        logging.info(f"PDWD-1s: Start distribution true: {rv_true.label} alter: {rv_alter.label}")
