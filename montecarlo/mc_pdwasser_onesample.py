...
from pdwasser import PDWasser_onesample
import pandas as pd
import logging
...
def run_mc(rvs, args):
    ...
    outputfilename = f"pdwasser_dim={args.dim}_n={args.n}_persistence_dim={args.persistence_dim}_order={args.order}_p={args.p}_null={null_distr_label}.csv"
    outputfilepath = os.path.join(args.output_dp, outputfilename)
    results = []

    logging.info(f"IM-1s: Start true distribution: {rv_true.label} n={args.n} dim={args.dim} persistence_dim={args.persistence_dim}")

    pdwasser = PDWasser_onesample(
        n=args.n,
        dim=args.dim,
        significance_level=args.alpha,
    ...
    # train TopoTest
    pdwasser.fit(rv=rv_true, n_signature=args.n_signature, n_test=args.n_test)
    for rv_alter in rvs:
        ...
        tt_out = pdwasser.predict(samples)
        results.append(
            data_row(
                args=args,
                ...
                accpecth0=tt_out[0],
                pvals=tt_out[1],
                threshold=pdwasser.representation_threshold,
            )
        )
