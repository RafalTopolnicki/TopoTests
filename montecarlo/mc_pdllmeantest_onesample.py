...
from pdllmeantest import PDLLMeanTest_onesample
import pandas as pd
import logging
...
def run_mc(rvs, args):
    outputfilename = f"dpllmeantest_dim={args.dim}_n={args.n}_persistence_dim={args.persistence_dim}.csv"
    ...
    for rv_true in rvs:
        ...
        if df_done is not None and rv_true_done:
            logging.info(f"LLMean-1s: All with true distribution: {rv_true.label} n={args.n} dim={args.dim} persistence_dim={args.persistence_dim} done!")
            continue

        pdlll_test = PDLLMeanTest_onesample(
            n=args.n,
            dim=args.dim,
            significance_level=args.alpha,
        ...
        # train TopoTest
        pdlll_test.fit(rv=rv_true, n_signature=args.n_signature, n_test=args.n_test)
        for rv_alter in rvs:
            if df_done is not None and np.any(np.logical_and(df_done.true_dist == rv_true.label, df_done.alter_dist == rv_alter.label)):
                logging.info(f"LLMean-1s: Skip distribution true: {rv_true.label} alter: {rv_alter.label}")
                continue
            else:
                logging.info(f"LLMean-1s: Start distribution true: {rv_true.label} alter: {rv_alter.label}")
                ...
                tt_out = pdlll_test.predict(samples)
                results.append(
                    data_row(
                        args=args,
                        ...
                        accpecth0=tt_out[0],
                        pvals=tt_out[1],
                        threshold=pdlll_test.representation_threshold,
                    )
                )
