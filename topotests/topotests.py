...
class TopoTest:
    ...
    def fit(self, rv, n_signature, n_test):
        ...
        samples = [
            rv.rvs(self.sample_pts_n).reshape(-1, self.sample_pt_dim) * self.scaling
        ...
        representation_test_skip = int(
            len(self.representation_signature[0]) / self.n_points_to_save
        )
        self.representation_signature = self.representation_signature[
            ::representation_test_skip
        ]

        if self.method not in ["ecc", "ecc-exact"]:
            dmin, dmean, dmax, dq = self.aggregate_distances(
                self.representation_distance
            )
            ...
            self.representation_threshold = {
                "mean": np.quantile(
                    self.representation_distance, 1 - self.significance_level
                )
            }
        self.fitted = True
    ...
    def predict(self, samples, label):
        ...
        representation_test_skip = int(
            len(representation_test[0]) / self.n_points_to_save
        )
        self.representation_predict[label] = [
            rep[::representation_test_skip] for rep in representation_test
        ]
        ...
        res = {
            "mean": [
                dp < self.representation_threshold["mean"]
                for dp in self.representation_distance_predict
            ]
        }
        return res
    ...
    def save_representation(self, filename):
        representation_test_skip = int(
            len(self.representation.xs) / self.n_points_to_save
        )
